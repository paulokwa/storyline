...
export default function ObjectsTab({
    ...
    async function handleDeleteObject() {
        ...
        setIsSaving(true)
        const supabase = createClient()
        const { error } = await supabase.from('objects' as any).delete().eq('id', selectedId)
    ...
    async function handleCreateObject() {
        ...
        if (data) {
            setLocalObjects((prev: any[]) => [...prev, data])
                        ...
                        <div className="max-w-3xl mx-auto px-6 py-8 sm:px-12 sm:py-16 space-y-12">
                            <div className="space-y-6">
                                <div className="flex items-center gap-4">
                                    ...
                                    <button onClick={handleDeleteObject} className="p-2 hover:bg-red-50 text-stone-300 hover:text-red-400 rounded-full transition-all"><Trash2 className="w-3.5 h-3.5" /></button>
                                </div>
                            ...
                            <div className="pt-16 flex items-center justify-between">
                                <div className="flex items-center gap-6">
                                    <div className="flex flex-col gap-1 text-[9px] uppercase tracking-widest text-slate-300 font-bold"><span>Discovery</span><span suppressHydrationWarning className="text-[10px] font-serif italic text-slate-400 normal-case font-normal">{new Date(selectedObject.created_at).toLocaleDateString()}</span></div>
