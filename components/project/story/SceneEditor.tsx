...
const SceneEditor = forwardRef<SceneEditorRef, SceneEditorProps>(({
    ...
    useEffect(() => {
        ...
        if (!dismissed && writingMode === 'screenplay') {
            setShowTabHint(true)
        }
    }, [writingMode])
    ...
    const editor = useEditor({
        ...
        onUpdate: ({ editor, transaction }) => {
            ...
            // Track active block type for hint
    ...
    return (
        <div className={cn(
            ...
            >
                ...
                {showTabHint && writingMode === 'screenplay' && (
                    <div className="absolute top-0 right-0 -translate-y-12 animate-in fade-in slide-in-from-top-4 duration-500">
                        <div className="flex items-center gap-3 bg-white/90 backdrop-blur-md border border-indigo-100 px-4 py-2 rounded-2xl shadow-lg ring-1 ring-indigo-500/10">
                            ...
                            <button 
                                onClick={dismissHint}
                                className="ml-2 p-1 text-slate-400 hover:text-slate-600 hover:bg-slate-50 rounded-lg transition-colors"
                            >
                                <X className="w-3 h-3" />
                            </button>
                        </div>
                ...
                {/* Dynamic Block Type Ghost Text */}
                {writingMode === 'screenplay' && activeBlockType && (
                    <div className="absolute bottom-4 right-6 pointer-events-none select-none animate-in fade-in duration-700">
                        <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.3em] text-slate-300 transition-all">
                             <div className="w-1.5 h-1.5 rounded-full bg-slate-200" />
                             {activeBlockType.replace('screenplay', '').replace(/([A-Z])/g, ' $1').trim()}
                        </div>
                    </div>
                )}
