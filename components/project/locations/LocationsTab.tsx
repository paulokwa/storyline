...
import { MapPin, Plus, Search, ChevronRight, PenTool, Hash, Loader2, Trash2, Pencil } from 'lucide-react'
            ...
            <div className={cn("w-full md:w-80 md:min-w-80 bg-[#f5f4ef] flex flex-col border-r border-slate-200/50 transition-all duration-300", selectedId && "hidden md:flex")}>
                ...
                <div className="flex-1 overflow-y-auto px-4 pb-10 space-y-1">
                    {localLocations.map((loc: any) => (
                        <div key={loc.id} onClick={() => setSelectedId(loc.id)} className={cn("w-full flex items-center gap-3 px-4 py-4 rounded-2xl transition-all duration-300 text-left cursor-pointer group", selectedId === loc.id ? "bg-white shadow-sm ring-1 ring-slate-100" : "hover:bg-white/40 text-slate-500 hover:text-slate-800")}>
