...
| Google OAuth login/signup | Needs retest | AI agent | 2026-05-11 | `Continue with Google` was added to login and signup using Supabase OAuth and the existing `/api/auth/callback` route. TypeScript, focused ESLint, diff check, and `npm run build` passed after rebasing onto the latest `main`; browser validation is blocked until Supabase and Google Cloud OAuth settings are configured. |
