...
| New/reply notification behaviour | Needs retest | AI agent | 2026-05-07 | Audit confirmed unread comment state is derived from unread `collaborator_feedback` rows. Bell clicks and notification detail views mark them read, but opening the comments panel via notification routing (`?feedback=1`) or activating a thread in the panel does not currently clear `read_at`. Manual browser validation is still needed before changing this policy. |
| Shared-project notification dedupe after remove/re-add | Needs retest | AI agent | 2026-05-07 | SQL now backfills one canonical `project-shared:<projectId>:<userId>` event key and reuses that key for future `project_shared` inserts. Manual validation needed: share a project, remove the collaborator, re-add the same account, and confirm the bell does not accumulate duplicate share notifications unless the original notification was deleted. |
...
| Notifications transfer-guidance creation does not 403 | Passed | User | 2026-05-06 | User retested live/incognito after switching transfer-guidance notification creation from direct browser insert to the create_notification RPC. No POST /rest/v1/notifications 403 console error appeared. The notification may not appear every incognito session because database event_key deduplication is expected. |
