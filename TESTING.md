...
| Test | Status | Tested by | Date tested | Notes |
|---|---|---|---|---|
| Signup verification link while another user is signed in | Needs retest | AI agent | 2026-05-06 | Failed in production before this patch: a reused/expired signup link could reopen the already-signed-in account's Library instead of showing neutral auth guidance. Retest: stay signed in as Account A, open Account B's invalid or reused signup verification link, and confirm the local session is cleared and the app ends on `/login?verification=already-used`. |
| Reused signup verification link shows explicit guidance | Needs retest | AI agent | 2026-05-06 | Failed in production before this patch: `/library?error=Invalid_Or_Expired_Token#error=access_denied&error_code=otp_expired...` could remain in the URL. Retest both query-param and hash-param variants: click the same signup verification link twice and confirm the second visit lands on `/login?verification=already-used` with the friendly guidance instead of any raw `/library` auth error URL. |
