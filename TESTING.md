...
| Editor current-scene find Block 3B Safe Pass 2 | Needs retest | AI agent | 2026-05-02 | `SceneEditor.tsx` now has a local current-scene find panel with a visible `Find` control, scoped `Ctrl/Cmd + F`, case-insensitive plain-text matching, `1 of N` status, and next/previous navigation. Manual browser validation is still needed for prose/screenplay matching, no-results state, scene-switch reset, Focus Mode usability, Typewriter Mode regression, and viewer/read-only safety. |
