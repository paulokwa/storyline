...
| Test | Status | Tested by | Date tested | Notes |
|---|---|---|---|---|
| Smart/Manual Context Phase 1 schema and types | Needs retest | AI agent | 2026-05-07 | Migration and TypeScript type support added for `ai_context_mode` and `exclude_from_ai`. Linked Supabase migration `20260507210000` is now applied and a service-role Supabase client query confirmed `user_api_keys.ai_context_mode` is visible through PostgREST. Browser validation is still needed. |
| Smart Context settings persistence | Needs retest | AI agent | 2026-05-07 | `/api/ai/preferences` now saves and returns `ai_context_mode`, and Settings has a Smart Context / Manual Context control. Verified statically with TypeScript, focused lint, and build. Browser/database validation is still needed to confirm saves persist for real users. |
