...
| Incomplete local setup draft hidden from other same-browser users | Needs retest | AI agent | 2026-05-18 | Incomplete setup drafts now use user-scoped localStorage keys. Browser validation: on the same browser/profile, account A starts a local project setup and exits before creating it; account B signs in and should not see account A's `Resume your setup` card. Sign back into account A and confirm their draft still resumes. |
...
| Project Settings premise/tone helper text visual QA | Needs retest | Codex | 2026-05-25 | Helper copy was changed from input-like bordered boxes to lightweight rows with right-aligned "AI Partner Context" badges. Browser visual QA still needed at desktop and narrow/mobile widths. |
...
| Google OAuth signup receives Free Trial AI grant | Needs retest | AI agent | 2026-05-18 | Google OAuth user `paulotenguk@gmail.com` had default AI settings but `ai_trial_accounts.status = disabled`, `grant_count = 0`, and no trial balance. `app/api/auth/callback/route.ts` now evaluates the trial grant after successful OAuth code exchange. Browser retest after deploy: sign up or sign in through Google, then confirm `ai_trial_accounts.grant_count > 0`, `status = active`, and nonzero `remaining_micros`. |
...
| Bulk delete selected structure nodes | Passed | User | 2026-05-19 | Bottom action bar appears when nodes are selected. Count correct, amber confirmation dialog works, descendants removed cleanly, selection clears after delete. |
| Type picker popover on chapter + button | Passed | User | 2026-05-19 | Popover renders with solid background via portal. Scene and Sub-chapter options present and functional. Episode/Act + buttons fire immediately without popover. |
| Auto-rename on node creation | Passed | User | 2026-05-19 | Title field immediately focused after node creation. Placeholder name fully pre-selected — typing replaces it in one gesture. |
| Empty droppable gaps closed | Passed | User | 2026-05-19 | Empty parent chapters collapse flush at rest. Drop zone expands correctly during drag-over. |
| Action buttons always visible in flex flow | Passed | User | 2026-05-19 | +, pencil, and trash icons visible on hover with long titles. Title truncates with ellipsis before icons — no overlap. |
...
| Export nested chapter ordering | Needs retest | AI agent | 2026-05-20 | `buildDepthFirstNodes` replaces global order_index sort. Given Chapter 1 → Nested Chapter A → Scene 1 / Scene 2 / Chapter 2 → Scene 3, export order must be: Chapter 1, Nested Chapter A, Scene 1, Scene 2, Chapter 2, Scene 3. Verify in at least one format (Markdown recommended). |
| Export nested chapter heading depth — Markdown | Needs retest | AI agent | 2026-05-20 | Root chapter (depth=0) → `##`. Nested chapter (depth=1) → `###`. Scene under nested chapter (depth=2) → `####`. Verify nested chapter does NOT render at the same level (`##`) as root chapters. |
| Export nested chapter heading depth — HTML/EPUB | Needs retest | AI agent | 2026-05-20 | Same depth formula as Markdown: depth+2 capped at 6. Root chapter → `<h2>`, nested chapter → `<h3>`. Verify rendered output visually. |
| Export nested chapter heading depth — DOCX | Needs retest | AI agent | 2026-05-20 | Root chapter (depth=0) → Heading 1. Nested chapter (depth=1) → Heading 2. Verify DOCX styles applied correctly in Word/LibreOffice. |
| Export flat novel regression — no heading change | Needs retest | AI agent | 2026-05-20 | Flat novel (no sub-chapters): Chapter → `##`, scene title (if shown) → `###`. TV script: episode → `##`, act → `###`, scene title → `####`. Neither format should have any visible regression vs pre-fix output when no sub-chapters exist. |
...
| 2026-05-17 | AI agent | Magic Detect long-wait progress feedback static verification | Passed | `npx tsc --noEmit --pretty false`, focused ESLint for `components/new-project/ImportWizard.tsx`, `git diff --check`, and `npm run build` passed after adding elapsed-time waiting messages and capped progress movement while the AI import request is pending. `netlify build --context production` completed Next build/function bundling, then hit the documented local Windows static-publish limitation. Browser verification on a slow/large manuscript is still recommended. |
| 2026-05-17 | AI agent | Import wizard AI preview cache and modal containment static verification | Passed | `npx tsc --noEmit --pretty false`, focused ESLint for `components/new-project/ImportWizard.tsx`, `git diff --check`, and `npm run build` passed after caching Magic Detect previews separately from manual split previews and constraining the confirmation modal with `100dvh`. Follow-up moved modal scrolling to an inner panel so the scrollbar stays inside the rounded shell; `netlify build --context production` completed Next build/function bundling, then hit the documented local Windows static-publish limitation. Signed-in browser verification is still recommended. |
