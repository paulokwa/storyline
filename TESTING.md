...
| Test | Status | Tested by | Date tested | Notes |
|---|---|---|---|---|
| Signup verification link while another user is signed in | Needs retest | AI agent | 2026-05-06 | Partial pass found in production: the reused/expired link now reaches the friendly login message, but normalization could still finish on a Netlify deploy permalink host. Retest: stay signed in as Account A, open Account B's invalid or reused signup verification link, and confirm the local session is cleared and the final URL is the canonical production host `/login?verification=already-used` with no hash. |
| Reused signup verification link shows explicit guidance | Needs retest | AI agent | 2026-05-06 | Partial pass found in production: the friendly banner now shows, but repeated visits could still normalize to `69fbd2be4ea6150008706c9e--storyline-paulokwa-v2.netlify.app/login?verification=already-used#error=...`. Retest query-param and hash-param variants from both Library and Login: every reused/expired visit must end on `https://storyline-paulokwa-v2.netlify.app/login?verification=already-used` with no raw Supabase hash or deploy-permalink host left visible. |
| Password reset email uses clean production redirect URL | Needs retest | AI agent | 2026-05-06 | Failed in production before this patch because a live recovery email used `redirect_to=http://localhost:3000/reset-password`. Retest: request a production password reset, inspect the decoded Supabase link, and confirm it contains no `localhost` and no Netlify branch/deploy host. |
...
| Notifications 403 gone in incognito / fresh session | Needs retest | AI agent | 2026-05-06 | `OpenProjectButton.tsx` no longer calls `supabase.from('notifications').insert(...)` directly (no INSERT RLS policy). Now calls `supabase.rpc('create_notification', {...})` with an event_key for deduplication. Browser validation needed: open the library in incognito, confirm no `POST /rest/v1/notifications 403` appears in the console, and confirm the `local_transfer_guidance` notification still appears in the bell. |
